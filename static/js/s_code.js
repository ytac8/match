...
/* code version 6.4 2015.09.01 */
  ...
  var s_account = 'rcrttraining32dev'
  var s = s_gi(s_account)
  /** ************************ CONFIG SECTION **************************/
  /* You may add or alter any code config here. */
  s.charSet = 'UTF-8'
  s.cookieDomainPeriods = '1'
  /* Conversion Config */
  s.currencyCode = 'JPY'
  ...
  s.linkInternalFilters = 'javascript:,https://localhost'
...
made when instructed to do so by your account manager. */
  ...
  s.siteID = 'match'					// leftmost value in pagename
...
*****************************************/
  ...
  /****************************************
	...
	* 新規・再来訪の取得
	*****************************************/
    s.prop3 = s.eVar3 = s.getNewRepeat(60, 's_nr')
    /****************************************
	* 初回、前回からの訪問間隔
	*****************************************/
    s.getFirstRepeat(s.prop3)
    /****************************************
	* VOSの取得
	*****************************************/
    if (!s.campaign) {
...
*/
  s.getCustomVid_all = function (cn) {
    ...
    if (s.bl_smart) { ret = s.bl_smart } else {
      var e = new Date(); ret = e.getTime() + '' +
(Math.random() * 10000000000000000)
      e.setTime(e.getTime() + (5 * 365 * 24 * 60 * 60 * 1000))
      s.c_w(cn, ret, e)
    } if (!s.c_r(cn)) { ret = '' } return ret
  }
